...
class World:
    ...
    def start(self, num_players, num_epidemics):
        ...
        city_cards = [CityCard(city) for city in self.cities.keys()]
        bucket_size = round(len(city_cards) / num_epidemics)
        buckets = [city_cards[i:i+bucket_size] for i in range(0, len(city_cards), bucket_size)]
        buckets = [shuffle(bucket+[EpidemicCard()]) for bucket in buckets]
        self.player_deck = deque([card for card in bucket for bucket in buckets])
    ...
    def draw_infection_card(self, infections=1):
        ...
        if sum([disease.outbreak_count for disease in self.diseases.values()]) > config['max_outbreaks']:
